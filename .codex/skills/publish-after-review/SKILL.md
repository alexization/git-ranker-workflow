...
이 skill의 목적은 latest verification evidence를 끝낸 결과를 open PR로 빠르게 공개하는 것이다. 이름은 남아 있지만 canonical timing은 `verification -> open PR publish -> optional review`다.

## 언제 사용하나

- latest verification report 또는 verification summary가 최신이다.
...
1. latest verification evidence가 현재 diff와 일치하는지 다시 확인한다.
2. PR body file은 section당 짧은 문단 하나 또는 1~3개 bullet만 채우고, exec plan 경로나 raw command literal은 넣지 않는다.
3. detailed evidence는 exec plan이나 linked artifact에 둔다.
4. 사용자가 draft를 명시적으로 요청하지 않았다면 open PR을 생성한다.
5. 사용자가 draft를 명시적으로 요청했다면 `gh pr create --draft --base develop --body-file <body-file>`로 draft PR을 생성한다.
...
- stale verification evidence나 stale review verdict로 publish하지 않는다.

...
- 관련 workflow 문서를 확인한다.
  - 계약 동기화를 직접 설명하는 exec plan
  - 필요 시 `docs/operations/verification-contract-registry.md` 같은 control-plane source of truth 문서
...
- 현재 workflow 저장소는 앱 계약 복제본이나 generated mirror를 유지하지 않으므로, 기본 출력은 “canonical backend spec 확인 + consumer/doc sync 결과”다.
...
- workflow가 앱 계약 복제 문서를 유지하지 않는다면 그 사실과 no-op 사유
...
- workflow에 앱 계약 복제본이 없는데도 존재하는 것처럼 체크리스트를 작성하지 않는다.
...
- Workflow review:
  - active exec plan
  - `docs/operations/verification-contract-registry.md` 필요 시
- Verification:
  - `npx tsc --noEmit -p git-ranker-client/tsconfig.json`
- Note:
  - workflow에는 앱 계약 tracked mirror가 없어 no-op
```

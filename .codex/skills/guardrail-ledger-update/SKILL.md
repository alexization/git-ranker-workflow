...
- latest review verdict가 `approved`, `blocked` 중 하나로 고정됐거나 review 불가 사유가 정리됐다.
...
- `changes-requested`가 latest review 상태였다면 feedback close-out이 아니라 repair loop로 되돌렸다는 근거

...
description: review가 실제로 필요할 때 latest verification report를 바탕으로 reviewer minimum context를 묶고, open PR 기준 review evidence를 남긴다. implementer와 reviewer 세션 분리를 유지한 채 review를 시작해야 할 때 이 skill을 사용한다.
---
...
- latest verification report가 `passed`가 아니면 review로 넘기지 않는다.

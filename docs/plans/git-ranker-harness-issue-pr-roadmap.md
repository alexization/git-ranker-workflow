...
SKILL은 기본적으로 `git-ranker-workflow/skills/<skill-name>/SKILL.md` 구조로 관리한다.
...
| `GRW-S01` | `GRW-01` | `skills/` 하위만 | skill은 문서형 자산부터 시작하고, 처음부터 실행 스크립트를 넣지 않는다. 각 skill 폴더의 필수 파일은 `SKILL.md` 하나다. |
...
| `GRW-S02` | `GRW-S01`, `GRW-03`, `GRW-04` 권장 | `skills/` 하위만 | coordination skill만 작성한다. 코드 생성이나 검증 실행 skill은 아직 넣지 않는다. 각 skill에는 병렬 수행 시 ownership 규칙을 반드시 적는다. |
...
| `GRW-S03` | `GRW-S01`, `GRC-04`, `GRW-06` | `skills/` 하위만 | 이 단계의 skill은 이미 만들어진 runtime/query/test를 재사용하는 설명서여야 한다. 새 인프라를 발명하지 않는다. |
...
| `GRW-S04` | `GRW-S01`, `GRW-09` 권장 | `skills/` 하위만 | reliability skill은 체크리스트 중심으로 시작한다. 자동 복구나 자동 triage 구현은 이 Issue의 범위가 아니다. |
...
- workflow 저장소에 `skills/` 디렉터리 구조를 만든다
- `skills/README.md`를 추가한다
- `skills/_template/SKILL.md`를 만든다
...
- `skills/ranking-read-harness/SKILL.md`
- `skills/playwright-browser-qa/SKILL.md`
- `skills/promql-logql-evidence/SKILL.md`
...
- `skills/batch-failure-triage/SKILL.md`
- `skills/github-rate-limit-investigation/SKILL.md`
- `skills/doc-gardener/SKILL.md`

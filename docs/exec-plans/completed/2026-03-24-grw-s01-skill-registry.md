...
`GRW-S02`, `GRW-S03`, `GRW-S04`는 모두 `.codex/skills/` 아래에 새 skill을 추가하는 작업이다. 먼저 registry와 authoring 규칙을 고정해야 후속 skill을 같은 기준으로 검토하고 확장할 수 있다.

## Scope

- `.codex/skills/` 디렉터리 추가
- `.codex/skills/README.md`에 registry 구조, naming 규칙, 지원 디렉터리 규칙 정리
- `.codex/skills/authoring-rules.md`에 최소 authoring 규칙 정리
...
- `find .codex/skills -maxdepth 2 -type f | sort`
  - 결과: `.codex/skills/README.md`, `.codex/skills/authoring-rules.md` 두 문서만 생성된 것을 확인했다.
- `cat .codex/skills/README.md`
  - 결과: 기본 구조를 `.codex/skills/<skill-name>/SKILL.md`로 고정하고, 지원 디렉터리는 각 skill 폴더 아래 선택 사항으로 두는 규칙을 확인했다.
- `cat .codex/skills/authoring-rules.md`
  - 결과: 고정 템플릿 없이도 각 skill이 다뤄야 할 최소 항목과 review checklist를 확인했다.
- `rg -n "GRW-S01|authoring 규칙|\.codex/skills/authoring-rules.md" docs/product .codex/skills docs/exec-plans/completed/2026-03-24-grw-s01-skill-registry.md`
  - 결과: `GRW-S01` 명칭과 산출물이 roadmap, catalog, skill 문서, 완료 기록에 일관되게 반영된 것을 확인했다.
- `rg -n "_template" docs/product .codex/skills`
  - 결과: `docs/product/`와 `.codex/skills/` 아래 source of truth 기준 문서에는 `_template` 언급이 남지 않았다.

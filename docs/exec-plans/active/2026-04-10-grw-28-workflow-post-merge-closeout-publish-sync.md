...
- Command: `sed -n '1,420p' docs/exec-plans/completed/2026-04-08-grw-18-workflow-pilot-closeout-reconciliation.md`
  - Status: `passed`
  - Evidence: `GRW-18` completed historical record가 post-merge close-out sync, feedback close-out, active queue terminal state를 함께 기록한다.
- Command: `sed -n '1,220p' docs/exec-plans/active/2026-04-10-grw-28-workflow-post-merge-closeout-publish-sync.md`
  - Status: `passed`
  - Evidence: `GRW-28` active exec plan이 issue `#77`, write scope, verification, blocker-sharing publish path를 현재 diff 기준으로 고정한다.
...
- Failure summary: 없음
- Next action: canonical review runtime blocker를 기록하고 draft blocker-sharing exception path로 publish한다.
...
- latest verification report를 채운다.
- publish path를 `approved` 또는 `blocker-sharing draft exception` 중 하나로 고정한다.
- publish result를 exec plan에 기록한다.

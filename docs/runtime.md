...
8. `verify`로 acceptance command 실행
...
- `verify`는 active phase 또는 지정 phase의 `acceptance.commands`를 실행해 run evidence를 남긴다.
...
`init`는 로컬 git 저장소의 `core.hooksPath`를 `.githooks`로 맞추고, `doctor`는 이 설정이 어긋나면 실패한다.
...
- `reopen`은 `failed`, `blocked`, `review_ready`, `completed` task를 다시 `approved`로 되돌리고 repair loop를 재시작한다.

...
| `NODE_ENV` | 필수 런타임 전제 | [middleware.ts](../../git-ranker-client/src/middleware.ts), [auth/callback/page.tsx](../../git-ranker-client/src/app/auth/callback/page.tsx), [web-vitals-reporter.tsx](../../git-ranker-client/src/shared/components/web-vitals-reporter.tsx), Sentry 설정 파일 | framework 기본값 | CSP, 개발 로그, analytics/Sentry enablement를 가른다 |
...
| `www.googletagmanager.com`, `www.google-analytics.com`, `region1.google-analytics.com` | [layout.tsx](../../git-ranker-client/src/app/layout.tsx), [middleware.ts](../../git-ranker-client/src/middleware.ts) | Google Analytics script/load/connect | 분석 데이터만 누락 |
| `fonts.gstatic.com`, `fonts.googleapis.com` | [layout.tsx](../../git-ranker-client/src/app/layout.tsx), [middleware.ts](../../git-ranker-client/src/middleware.ts) | `JetBrains_Mono` font 로드와 CSP 허용 | 폰트 fallback 또는 build/runtime 재현성 저하 |
...
현재 middleware가 모든 응답에 아래 정책을 주입한다.

- `Content-Security-Policy`
  - `script-src`: self, inline/eval, Google Tag Manager
  - `style-src`: self, inline, `fonts.googleapis.com`
  - `font-src`: self, `fonts.gstatic.com`
  - `img-src`: self, data/blob, GitHub avatar/GitHub, Google Analytics
  - `connect-src`: dev에서는 `http://localhost:8080`과 analytics host 허용, prod에서는 analytics host만 추가 허용
- `Referrer-Policy: strict-origin-when-cross-origin`
- `Permissions-Policy: camera=(), microphone=(), geolocation=(), interest-cohort=()`

`next.config.ts`는 remote image host를 `github.com`, `avatars.githubusercontent.com` 두 곳으로 제한한다.
...
- `NEXT_PUBLIC_API_URL` fallback 사용 여부가 파일마다 다르다. 특히 [api-client.ts](../../git-ranker-client/src/shared/lib/api-client.ts)는 fallback이 없어 가장 민감하다.
- root layout은 local font와 함께 `next/font/google`의 `JetBrains_Mono`를 사용하므로, build 재현성 관점에서는 여전히 외부 font dependency가 남아 있다.
- OAuth callback은 backend 설정상 `/auth/callback`이 canonical이지만, client에는 동일 구현의 `/oauth2/redirect` route가 남아 있다.

## Harness Notes

- `GRC-03` build/runtime 하네스 친화화는 `NEXT_PUBLIC_API_URL`의 강제성, Google font 제거 여부, hard-coded analytics dependency를 우선 다뤄야 한다.
- ranking harness는 backend API, GitHub avatar CDN, middleware locale redirect를 함께 고려해야 한다.

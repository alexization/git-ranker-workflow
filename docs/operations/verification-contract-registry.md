...
- optional 또는 conditional command는 생략 가능하지만, 생략 시에도 trigger 부재 또는 제외 이유를 verification evidence에 남겨야 한다.
- reviewer에게 넘기는 verification evidence는 필수 산출물이다. evidence가 없거나 필드가 비어 있으면 verification은 끝난 것으로 보지 않는다.
...
| overall | `passed` | 모든 required command가 `passed`이고 evidence 필드가 완전하다 | `Reviewing` |
...
- 로컬 worktree가 없으면 GitHub의 공식 file fetch 또는 issue metadata 확인으로 대체하되, 어떤 remote source를 썼는지 evidence에 적는다.
...
- lint, typecheck, build가 모두 성공하고 required `NEXT_PUBLIC_*` env 전제가 verification evidence에 남는다.
...
canonical verification evidence는 exec plan이나 별도 verification artifact 중 최소 한 곳에 남긴다. PR 본문에는 필요하면 최종 상태와 핵심 공백만 요약한다.

### Compact Verification Summary

모든 required command가 통과했고 skipped/blocked nuance가 없으면 compact summary를 기본값으로 쓸 수 있다.
...
- Ran:
  ...
  - `rg -n "reader-first|verification evidence" docs/operations .github .codex/skills`
  - `git diff --check`
- Evidence:
  - reader-first body 규칙, template, skill wording이 함께 정렬됐다.
- Failure or skipped summary: 없음
- Next action: open PR publish
```

### Detailed Verification Report

아래 중 하나라도 해당하면 detailed report가 필요하다.

- required command가 `failed`, `blocked`, `skipped`다
- conditional command가 trigger되었거나 skip reason이 후속 판단에 중요하다
- repair history나 환경 전제를 command 단위로 남겨야 한다
...
- `Contract profile`
- `Overall status`
- `Preconditions` 또는 relevant env/runtime 전제 필요 시
...
- compact summary는 "무엇을 실행했고 최신 결과가 무엇인지"를 한 번에 보여야 한다.
- detailed report를 쓸 때는 conditional command 생략 사유까지 command 단위로 남긴다.
- reviewer는 최신 verification evidence 하나만 읽어도 현재 required command 상태를 판단할 수 있어야 한다.
- 이전 repair attempt의 실패 로그는 남길 수 있지만, 최종 evidence에는 "현재 최신 기준에서 어떤 명령이 통과했는가"가 다시 요약돼야 한다.
...
- reviewer가 "diff와 verification evidence가 불일치한다"고 판단했다.
...
- failure가 baseline command에 걸려 있었으면 그 아래 conditional command를 통과로 간주하지 않는다. 최신 verification evidence에서 다시 상태를 적어야 한다.
- reviewer handoff 전에는 latest verification evidence 기준으로 모든 required command가 `passed`여야 한다.
...
review가 실제로 필요할 때 reviewer에게는 최소한 아래 입력이 함께 있어야 한다.
...
reviewer pool을 쓰는 경우에도 이 handoff surface를 그대로 fan-out한다. role prompt는 이 surface 위에 추가되는 focus일 뿐, minimum context를 없애거나 final verdict owner의 확인 책임을 덜어내는 근거가 아니다.

각 reviewer는 자기 역할에 필요한 subset만 읽을 수 있지만, final verdict owner는 latest verification evidence와 reviewer finding을 같은 revision 기준으로 정렬해야 한다.

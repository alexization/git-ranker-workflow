...
| 종료 상태 | 기준 | 다음 행동 |
| --- | --- | --- |
| `Planned` | issue의 reader-facing summary와 exec plan의 `Problem`, `Why Now`, `Scope`, `Non-scope`, `Write Scope`, `Verification`을 채울 수 있다. | GitHub issue를 만들고 active exec plan을 작성한다. |

...
| `즉시 실행 가능한 작업` | 단일 목표, 대상 저장소, 예상 산출물, write scope, verification을 현재 문서와 코드에서 고정할 수 있다. | `default lane` 또는 `guarded lane`을 고른 뒤 작업을 시작한다. | `Received -> Routed -> Scoped` 또는 `Received -> Routed -> Planned` |
...
4. 산출물과 비범위를 task brief 또는 exec plan에 바로 적을 수 있다.
5. 검증 명령 또는 문서 검토 절차를 적을 수 있다.
6. source of truth만으로 남은 선택을 처리할 수 있고, 새 사용자 선호를 물을 필요가 없다.

위 기준을 만족하면 곧바로 lane을 고른다. 작은 직접 요청은 `default lane`으로, tracked backlog나 고위험 작업은 `guarded lane`으로 보낸다.

## Execution Lane Selection

`즉시 실행 가능한 작업`은 아래 두 lane 중 하나로 고정한다.

| Lane | 선택 기준 | 즉시 해야 할 일 | 상태 전이 |
| --- | --- | --- | --- |
| `default lane` | 단일 저장소, clear scope, bounded write scope, 검증이 자명하고 rollback cost가 낮다. policy/API/schema/auth/CI/migration/destructive change가 아니다. | 짧은 task brief에 repo, 목표, write scope, verification을 적고 바로 구현한다. | `Routed -> Scoped` |
| `guarded lane` | tracked backlog, existing issue/exec plan, workflow/policy 변경, cross-repo planning, public contract/auth/schema/CI/migration/destructive change, 또는 사용자가 formal plan/review를 요구한다. | GitHub issue와 active exec plan을 만들고 범위를 formal하게 잠근다. | `Routed -> Planned` |

lane을 고른 뒤에도 아래 신호가 나오면 즉시 `default lane -> guarded lane`으로 승격한다.

- write scope가 커져 다른 저장소나 broad policy surface를 건드리게 됨
- public contract, auth, permission, CI, migration, destructive change가 뒤늦게 드러남
- 간단한 task brief만으로는 verification, non-scope, rollback risk를 설명할 수 없음
- 사용자가 추적 가능한 issue/plan/evidence를 명시적으로 요구함

## Ambiguity Interview Policy

### Interview Objectives

interview의 목적은 요청을 더 크게 정의하는 것이 아니라, 아래 여섯 항목을 잠그는 것이다.
...
- ambiguity가 해소되면 바로 task brief 또는 issue/exec plan 형식으로 요약해 범위를 고정한다.

### Interview Exit Conditions

| 종료 상태 | 기준 | 다음 행동 |
| --- | --- | --- |
| `Scoped` | repo, 목표, write scope, verification을 짧은 task brief로 잠글 수 있고 `default lane` 기준을 만족한다. | task brief를 남기고 구현을 시작한다. |
| `Planned` | issue의 reader-facing summary와 exec plan의 `Problem`, `Why Now`, `Scope`, `Non-scope`, `Write Scope`, `Verification`을 채울 수 있고 `guarded lane` 기준을 만족한다. | GitHub issue를 만들고 active exec plan을 작성한다. |
...
| "현재 catalog에 있는 request routing policy 작업을 진행해줘" | `즉시 실행 가능한 작업` | work item catalog와 시스템 문서가 이미 범위와 write scope를 제공하고 tracked backlog이므로 guarded lane으로 바로 잠글 수 있다. | issue 생성 후 exec plan 작성 |
| "`docs/operations/workflow-governance.md`의 오탈자 하나만 고쳐줘" | `즉시 실행 가능한 작업` | 단일 문서, low-risk, verification이 명확한 default lane 작업이다. | task brief를 남기고 바로 수정 |
...
- `default lane`은 task brief만으로 시작할 수 있다.
- `guarded lane`만 GitHub issue와 active exec plan을 만든다.

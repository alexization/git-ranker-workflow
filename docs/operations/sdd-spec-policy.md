...
3. 소크라테스 질문으로 문제 정의, 목표, 제약, 수용 기준을 좁힌다.
4. 각 라운드마다 확정된 점, 가정, 남은 공백을 짧게 요약한다.
5. Harness가 충분히 명확하다고 판단하면 spec 초안을 작성한다.
6. 사용자 승인을 받은 뒤에만 spec 상태를 `Approved`로 올린다.
7. 구현 중 scope가 바뀌면 별도 planning 문서를 만들지 말고 spec을 다시 승인받아 갱신한다.

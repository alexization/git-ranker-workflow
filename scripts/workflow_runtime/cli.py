...
from workflow_runtime.models import WorkflowError, emit


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Socratic SDD workflow control plane CLI")
    ...
    run_parser.add_argument("--imp-id")
    ...
    verify_parser.add_argument("--imp-id")
    ...
    reopen_parser.add_argument("--imp-id")
...
def field_value(payload: dict[str, Any], field: str) -> Any:
    ...
    for part in field.split("."):
        if not isinstance(value, dict) or part not in value:
            raise WorkflowError(f"unknown field: {field}")
        value = value[part]
    return value
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "plan":
            service.plan_task(args.task_id)
            emit({"status": "planned", "task_id": args.task_id})
            return 0

        if args.command == "run":
            if args.complete:
                service.complete_scope(args.task_id, args.imp_id, args.changed_path, args.note, args.evidence)
                emit({"status": "completed", "task_id": args.task_id})
                return 0
            if args.fail:
                code = service.fail_scope(args.task_id, args.imp_id, args.note)
                emit({"status": "failed", "task_id": args.task_id})
                return code
            service.start_scope(args.task_id, args.imp_id, args.note)
            emit({"status": "started", "task_id": args.task_id})
            return 0

        if args.command == "verify":
            code = service.verify_task(args.task_id, args.imp_id, args.verify_command or None, args.evidence)
            emit({"status": "passed" if code == 0 else "failed", "task_id": args.task_id})
            return code
        ...
        if args.command == "reopen":
            service.reopen_task(args.task_id, args.note, args.imp_id)
            emit({"status": "approved", "task_id": args.task_id})
            return 0
        ...
        if args.command == "status":
            if args.infer_active_task:
                ...
                print(service.infer_active_task() or "")
                return 0
            if args.check:
                ...
                tasks = sorted(
                    path.name
                    for path in service.tasks_dir.iterdir()
                    if path.is_dir() and (path / "state.json").exists()
                )
                emit({"status": "passed", "tasks": tasks})
                return 0
            if args.all:
                payload = {
                    "tasks": [
                        service.load_state(path.name)
            ...
            if args.field:
                value = field_value(payload, args.field)
        ...
        if args.command == "hook":
            ...
            service.normalize_changed_paths(changed_paths)
            state = service.load_state(args.task_id) if args.task_id else None
